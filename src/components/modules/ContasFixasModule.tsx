...
import { FixedExpense, PaymentMethod, Periodicity, ExpenseStatus } from '../../types';
...
export const ContasFixasModule: React.FC = () => {
  ...
  const [deleteItemTarget, setDeleteItemTarget] = useState<FixedExpense | null>(null);
      ...
      accessor: (r) => (
        <div>
          ...
          <span className="text-[11px] text-slate-400">
            Vence todo dia <strong className="text-slate-700 dark:text-slate-300">{r.dueDay}</strong> ({r.periodicity})
          </span>
        </div>
      ),
      sortable: true,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl relative">
            ...
            <h3 className="text-base font-bold text-slate-900 dark:text-white mb-4">
              {editingItem ? '✏️ Editar Conta Fixa' : '➕ Nova Conta Fixa'}
            </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Aluguel, Plano de Saúde, Internet Fibra"
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Dia do Vencimento</label>
                  <input
                    type="number"
                    min={1}
                    max={31}
                ...
                <div>
                  <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1">Periodicidade</label>
                  <select
                    value={periodicity}
              ...
              {paymentMethod === 'Cartão' ? (
                <div className="p-3 bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800 rounded-xl space-y-2">
                  ...
                  <p className="text-[10px] text-purple-600 dark:text-purple-300">
                    Ao salvar, o lançamento entrará automaticamente na fatura do cartão sem permitir duplicidade.
                  </p>
                </div>
              ) : (
      ...
      <SmartDeleteModal
        ...
        moduleType="geral"
        onConfirm={() => {
          if (deleteItemTarget) deleteFixedExpense(deleteItemTarget.id);
        }}
      />
    </div>
  );
